...
public class LopHocServerImpl implements LopHocServer {
	...
	public LopHoc getById(int maLopHoc) {
		LopHoc lopHoc= restTemplate.getForObject(crmUrl+"/lopHocs/"+maLopHoc, LopHoc.class);
		return lopHoc;
	}
	...
	public void save(LopHoc lopHoc) {
		...
		if(maLopHoc==0) {
			restTemplate.postForEntity(crmUrl + "/lopHocs", lopHoc, String.class);
		}else {
			restTemplate.postForEntity(crmUrl + "/lopHocs/update", lopHoc, String.class);
		}
	}

	@Override
	public void delete(int maLopHoc) {
		restTemplate.delete(crmUrl+"/lopHocs/"+maLopHoc);
